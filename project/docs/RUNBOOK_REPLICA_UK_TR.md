...
## Рестарт из админки бота

В `🛠 Админка` -> `🖥 Сервис` доступны кнопки:
- `🔁 Рестарт UK`
- `🔁 Рестарт TR`

Сценарий безопасный:
1. бот спрашивает подтверждение;
2. выполняет `restart-post` через `replica_ops.sh`;
3. показывает результат в чате админа.
